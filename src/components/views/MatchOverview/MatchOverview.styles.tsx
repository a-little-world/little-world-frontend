...
export const ProgressHeading = styled(Text)`
  color: ${({ theme }) => theme.color.text.heading};
`;

export const ProgressBody = styled(Text)`
  color: ${({ theme }) => theme.color.text.secondary};
`;

export const SearchToggleWrap = styled.div`
...
export const BadgeStrip = styled.ul<{ $expanded?: boolean }>`
  ...
  ${({ $expanded }) =>
    $expanded
      ? css`
          /* Everything at once: as many columns as fit, wrapping onto further rows. */
          grid-template-columns: repeat(auto-fill, minmax(${BADGE_WIDTH}, 1fr));
          row-gap: ${({ theme }) => theme.spacing.medium};
        `
      : css`
          /* One row of fixed-width medals. Start-aligned on purpose: centring a
             scroll container can put its first item out of reach. */
          grid-auto-flow: column;
          grid-auto-columns: ${BADGE_WIDTH};
          overflow-x: auto;
